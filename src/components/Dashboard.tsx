...
import { LogOut, TrendingUp, Send } from 'lucide-react';
...
export default function Dashboard({ userId, username, onLogout }: DashboardProps) {
  ...
  return (
    <VideoBackground>
      <div className="min-h-screen">
        ...
        <header className="flex justify-between items-center mb-6 md:mb-8">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <button
              onClick={onLogout}
              className="items-center gap-2 px-4 py-2 bg-slate-800/50 hover:bg-slate-700/50 text-slate-300 rounded-lg transition-all border border-slate-700 flex-shrink-0"
            >
      ...
      {showSend && (
        <SendModal onClose={() => setShowSend(false)} onSuccess={handleRefresh} userId={userId} />
      )}
      </div>
    </VideoBackground>
  );
